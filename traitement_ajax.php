...
if ($_GET && !$_POST) {
	...
	if (isset($_GET["repetition"])) {
		...
		if($resultat -> execute()){
			...
			$display = $resultat -> fetchAll(PDO::FETCH_ASSOC);

			$tables = $pdo -> query("SHOW TABLES");
			$tables = $tables -> fetchAll(PDO::FETCH_ASSOC);

			// traitement fichier historique
			$historique = fopen("historique.txt", "a");
			fwrite($historique, $bdd . " - " . $requete . "\r\n");
			fclose($historique);

			$msg = "<p style='padding: 10px; background-color: green;'>Voici le resultat de votre requete</p>";
			if(empty($display)){
